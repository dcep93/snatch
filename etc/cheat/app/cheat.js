...
router.post('/', function(req, res) {
	...
	var end = new Date();
	var response = {words: Array.from(foundWords), duration: (end-start), count: foundWords.size}
...
function findWords(words, letters) {
	...
	for (var i = 0; i < words.length; i++) {
		...
		var chain = sorted(word.toUpperCase());
		var t = trie;
...
function addWords(t, chain, remainingLetters, previous, foundWords, seenChains) {
	...
	for (var letter in t) {
		...
		if (letter === MAGIC) {
			...
			if (remainingLetters === undefined) {
				addWords(sub, newChain, remainingLetters, letter, foundWords, seenChains);
				...
				nextLetters[letter]--;
				addWords(sub, newChain, nextLetters, letter, foundWords, seenChains);
			}
		}
	}
}
